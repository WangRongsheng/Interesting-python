# -*- coding: utf-8 -*-
"""
Created on Wed Dec 13 00:34:29 2017
...
class SolarDate():
    ...
    def __str__(self):
        return 'LunarDate(%d, %d, %d, %d)' % (self.year, self.month, self.day, self.isLeapMonth)
class SolarDateTomorrow():
    def __init__(self):
        global solar_year
        global solar_month
        global solar_day
        global solar_weekday
        solar_year= (datetime.date.today()+datetime.timedelta(days=1)).year
        solar_month = (datetime.date.today()+datetime.timedelta(days=1)).month
        solar_day = (datetime.date.today()+datetime.timedelta(days=1)).day
        solar_weekday   = int(time.strftime("%w", time.localtime()))
    ...
    def __str__(self):
        return 'LunarDate(%d, %d, %d, %d)' % (self.year, self.month, self.day, self.isLeapMonth)
def getCalendar_tomorrow():
    solar = SolarDateTomorrow()
    LunarDate.fromSolarDate(solar_year, solar_month, solar_day)
    twitter = ChineseWord.month_lunar(lunar_isLeapMonth,lunar_month) \
    + ChineseWord.day_lunar(lunar_day)
    return twitter
def getCalendar_today():
    ...
    print(twitter)
    return twitter
def isNeedPromter():#遍历整个文件，查看是否有需要提醒的人，有的话，发送邮件
    p = getCalendar_tomorrow()
    print('明天就是'+p+'了')
    #直接通过明天的日期查找
    wb = openpyxl.load_workbook('Data.xlsx')
    Info = wb.get_sheet_by_name(wb.get_sheet_names()[1])
    for i in range(2,Info.max_row+1):
        dic = eval(Info.cell(row = i,column = 2).value)#将这一行的生日字典拿到
        if(p in dic):#需要发送邮件提醒
            names = dic[p]
            if('、' in names):
                names = names.replace('、','和')
            sendEmail(Info.cell(row = i,column = 1).value,Info.cell(row = i,column = 3).value,names)
            time.sleep(4)
        
                
    
def sendEmail(usename,Email,friend_name):
    msg_from = 'xxxxxxxx@qq.com'#我的邮箱
    passwd = 'xxxxxxxx'#授权码
    subject = '生日提醒'
    content = '亲爱的'+usename+'您好，明天您好友'+friend_name+'生日了，赶紧为TA准备一份精美的礼物吧'
...
def zhuce():
    ...
    print('请输入您的邮箱，便于我们提醒您：')
    Emile = input()
    print('请输入您想要保存的朋友的名字:',end = '')
    #print(datetime.date.today())
    name = input()
    print('生日输入示例：十月初三的生日格式为：10.3')
    print('请输入Ta的生日:',end = '')
    age = input()
    month,day = age.split('.')
    f = open('r.txt','a+')
    f.write(Emile+' '+name+' '+dic_month[month]+dic_day[day]+'\n')
    f.close()
def main():
    isNeedPromter()
    getCalendar_today()
