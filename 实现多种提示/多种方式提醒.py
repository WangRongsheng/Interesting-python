...
# 选择提醒方式
notice = '13'  # 1 = server酱  2 = 邮件  3 = 钉钉  12 = server酱+邮件  12=server酱+钉钉  23=邮件+钉钉  server酱就是天下第一
...
# 如果选择钉钉通知的话 请配置下方信息  钉钉机器人的 webhook
webhook_dingding = ''
...
def dingtalk(content):
    ...
    requests.post(url=webhook_dingding, headers=dd_headers, data=json.dumps(dd_message))
